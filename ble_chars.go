...
func setupBluetoothApplication(logger zerolog.Logger, coldBoot bool, vinLogger loggers.VINLogger, lss loggers.TemplateStore) (*service.App, context.CancelFunc, context.CancelFunc) {
	...
	vinChar.OnRead(func(_ *service.Char, _ map[string]interface{}) (resp []byte, err error) {
		defer func() {
			if err != nil {
				logger.Err(err).Ctx(context.WithValue(context.Background(), utils.LogToMqtt, "true")).Msgf("error retrieving VIN via BLE: %s", err)
			}
		}()
		...
		logger.Info().Ctx(context.WithValue(context.Background(), utils.LogToMqtt, "true")).Msg("Got VIN via BLE") //note we don't send the VIN to cloud logs for PII
		...
		if errSaveCfg != nil {
			logger.Err(errSaveCfg).Ctx(context.WithValue(context.Background(), utils.LogToMqtt, "true")).
	...
	protocolChar.OnRead(func(_ *service.Char, _ map[string]interface{}) (resp []byte, err error) {
		defer func() {
			if err != nil {
				logger.Err(err).Ctx(context.WithValue(context.Background(), utils.LogToMqtt, "true")).Msgf("Error retrieving Protocol: %s", err)
			}
		}()
		if lastProtocol != "" {
