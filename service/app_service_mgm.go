...
func (app *App) NewService(uuid string) (*Service, error) {
	...
	s.path = dbus.ObjectPath(fmt.Sprintf("%s/service%s", app.Path(), strings.ReplaceAll(s.UUID, "-", "_")[:8]))
...
func (app *App) AddService(s *Service) error {
	...
	app.Options.Logger.Trace().Msgf("Added GATT Service UUID=%s %s", s.UUID, s.Path())
